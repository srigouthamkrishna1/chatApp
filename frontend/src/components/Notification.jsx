    ...
    return (
        ...
        <div className="relative inline-block m-8 text-left">
            <div className="relative bg-slate-200 w-10 h-10 flex items-center justify-center rounded-full" onClick={() => { setIsOpen(!isOpen) }}>
                ...
                {(unreadNotifications?.length == undefined || unreadNotifications?.length == 0) ? null : <div className="absolute w-4 h-4 text-xs text-white bottom-7 left-6 bg-red-500 rounded-md flex items-center justify-center">{unreadNotifications?.length}+</div>}
            </div>
